...
extern int8_t jogDistCima ;
extern int8_t jogDistFrente ;
extern int8_t jogDistBaixo ;
extern int8_t jogDistAtras ;
...
void fMoveT( uint8_t param ) {
  ...
  if ( mPages[_P_MOVE].boutons[POS_OF_MOVE_D_AUTO] == _D_AUTO ) {
    ...
    Serial2.print(distance) ; Serial2.println (" F9000") ;
    ...
    /*
      ...
      Serial2.print(distance) ; Serial2.println (" F200") ;
    */
...
void handleAutoMoveT( uint8_t param) { // in Auto mode, we support long press to increase speed progressively, we cancel jog when released
...
#define AUTO_MOVE_REPEAT_DELAY 100
  ...
  if ( ( pressedBtn ) && (jogCmdFlag == false) ) {
    ...
    jogDistCima = 0 ;           // reset all deplacements
    jogDistFrente = 0 ;
    jogDistBaixo = 0 ;
    jogDistAtras = 0 ;
    ...
    switch ( typeOfMove ) {                                          // fill one direction of move
      ...
      case _YAP :  jogDistCima   = 1  ;  break ;
      case _XZP :  jogDistFrente   = 1 ;  break ;
      case _YAM :  jogDistBaixo   = 1  ;  break ;
      case _XZM :  jogDistAtras   = 1 ;  break ;
...
void posIni(uint8_t param) {
  if ( machineStatus[0] == 'I' || machineStatus[0] == 'A' ) {
#define posIniCMD "G90 G21 G01 X0 Y0 Z0 A0 F6000"
    Serial2.println(posIniCMD) ;
  } else {
...
// void fSetXYZ0(uint8_t param) {     // param contient le n° de la commande

//   memccpy ( printString , _SETXYZA_STRING , '\0' , 249);
//   sendStringToGrbl() ;
//   waitReleased = true ;          // discard "pressed" until a release
